...
const ProfileSection = ({ userId }) => {
  ...
  const updateProfileImage = async (file) => {
    ...
    formData.append("profileImage", file); // 파일을 "profileImage"라는 이름으로 추가

    try {
      const response = await fetch(
        "http://localhost:8253/mypage/profileImage/upload",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${getUserToken()}`, // 인증 토큰을 헤더에 추가
          },
      ...
      if (response.ok) {
        ...
        console.log("Received imageUrl:", result);
        console.log("Full response:", result);  // 전체 응답을 확인
        setProfileImg(result); // 서버에서 받은 이미지 URL을 상태에 저장
      } else {
  ...
  //       setProfileImg(imageUrl || defaultImg); // 서버에서 가져온 이미지 URL이 없으면 기본 이미지 사용
  //     } else {
  ...
  // useEffect(() => {
  //   fetchProfileImage(); // 페이지 로드 시 프로필 이미지만 가져옴
  // }, [userId]); // userId가 변경될 때마다 실행
  

  // // 프로필 이미지 조회 및 기본 이미지로 재설정하는 함수
  ...
  const resetProfileImage = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log("프로필 이미지를 서버에서 가져왔습니다:", data);
        setProfileImg(data.imageUrl || defaultImg); // 서버에서 받은 이미지 URL 또는 기본 이미지로 설정
      } else {
...
const fileChangeHandler = (e) => {
  ...
  if (file) {
    ...
    // 로그: FileReader가 시작될 때
    const reader = new FileReader();
    reader.onload = () => {
      console.log("FileReader result (base64):", reader.result);
      setProfileImg(reader.result); // 상태 업데이트
    };
    ...
    fetch(`http://localhost:8253/mypage/userInfo`, {
      ...
      .then((data) => {
        ...
        if (data.profileImageUrl) {
          setProfileImg(data.profileImageUrl); // 서버에서 받은 프로필 이미지 URL을 설정
        } else {
          resetProfileImage(); // 프로필 이미지를 서버로부터 가져옴 resetProfileImage
        }
      })
      .catch((error) => {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        const response = await fetch(`http://localhost:8253/mypage/userInfo`, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${getUserToken()}`,
            "Content-Type": "application/json",
        ...
        if (response.ok) {
          ...
          if (data.profileImg) {
            setProfileImg(data.profileImg); // 서버에서 받은 URL로 상태 업데이트
          } else {
        ...
        <div className={styles.contentsBox}>
          ...
          {modalActive && (
            <MypageModal
              ...
              onDefaultImage={resetProfileImage} //resetProfileImage
